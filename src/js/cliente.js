...
function consultarClientes(){
        ...
        fetch(`http://localhost:3000/clientes/buscar`)
        .then(response => {
            // Verifica si la respuesta es correcta (código de estado 200)
            if (!response.ok) {
                throw new Error('Clientes no encontrados'); // Lanza un error si el cliente no se encuentra
            }
    ...
    function loadData(data) {
        ...
        if (element) {
            table.rows.add(data.map(element => [
                ...
                element.telefonoCliente,
                element.correoElectronicoCliente
