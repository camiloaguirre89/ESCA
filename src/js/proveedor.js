...
function consultarProveedor(){
        ...
        fetch(`http://localhost:3000/proveedor/buscar`)
        .then(response => {
            // Verifica si la respuesta es correcta (código de estado 200)
            if (!response.ok) {
                throw new Error('Proveedor no encontrados'); // Lanza un error si el cliente no se encuentra
            }
